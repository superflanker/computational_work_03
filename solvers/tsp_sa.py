...
def simulated_annealing(tsp, work, time_limit):
    ...
    def init_temp(work):
        ...
        for k in range(num_sample):
            ...
            delta = tsp.dist(u, v) + tsp.dist(work.next(u), work.next(v)) - tsp.dist(u, work.next(u)) - tsp.dist(v,
                                                                                                                 work.next(
                                                                                                                     v))
            if delta > 0:
                avg_delta += delta
    ...
    while cur_time - start_time < time_limit:
        ...
        temp = math.pow(max_temp, 1.0 - (cur_time - start_time) / time_limit) * math.pow(min_temp, (
                    cur_time - start_time) / time_limit)
...
def three_opt_type134(tsp, work, temp):
    ...
    def eval_diff(tsp, work, u, v, w):
        ...
        if new - cur < best and (work.pos)[v] >= (work.pos)[u] + 3 and (work.pos)[w] >= (work.pos)[
            v] + 1:  # check node v and w
        ...
        if new - cur < best and (work.pos)[v] >= (work.pos)[u] + 2 and (work.pos)[w] >= (work.pos)[
            v] + 2:  # check node v and w
...
def solve(tsp_file, image_file, runs=10):
    ...
    for seed in range(0, runs):
        ...
        work.draw(tsp, image_file.format(seed))
